...
def cache_latest_rates(ds, **kwargs):
    redis_conn = redis.StrictRedis(host=os.environ.get('REDIS_HOST'), port=os.environ.get('REDIS_PORT'))
    ...
    for iso2, rate in pg_hook.get_records(latest_rates):
        print(f"{iso2}: {rate}")
        redis_conn.set(iso2, float(rate))
...
get_rates_task >> cache_latest_rates_task